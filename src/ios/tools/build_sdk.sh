...
proj="ADAL"
echo "Building $proj"
xcodebuild -UseModernBuildSystem=0 -workspace ADAL.xcworkspace -scheme "ADAL iOS" -configuration $BUILD_CONFIGURATION ARCHS="i386 x86_64" -sdk iphonesimulator VALID_ARCHS="i386 x86_64" ONLY_ACTIVE_ARCH=NO CONFIGURATION_BUILD_DIR="../build/emulator" clean build $DISABLE_COVERAGE $DISABLE_INSTRUMENT
xcodebuild -UseModernBuildSystem=0 -workspace ADAL.xcworkspace -scheme "ADAL iOS" -configuration $BUILD_CONFIGURATION ARCHS="armv7 armv7s arm64" -sdk iphoneos VALID_ARCHS="armv7 armv7s arm64" CONFIGURATION_BUILD_DIR="../build/device" clean build $DISABLE_COVERAGE $DISABLE_INSTRUMENT
